...
> **구현 증거 상태 (2026-08-02):** P0A/P0B/P1/P2/P3 source increments는 complete·accepted이고 P2/P3는 `origin/main`에 전달됐다. P4-A Offline/Synthetic foundation도 strict/versioned schema, canonical Decimal/SHA-256 binding, deterministic 8-stage runner, independent candidate payload, 20-edge disposition matrix, fail-closed geometry/normalization/warning 처리와 재현 가능한 metrics/report 범위에서 complete·independently accepted·committed·fresh-main fast-forward integrated·delivered 상태다. Final 25-path independent read-only review는 `ACCEPT_WITH_MINOR` (BLOCKER 0, MAJOR 0, MINOR 3)를 반환했고 review 전후 source-only 16-path digest `0d5c259f59293f35e4cf6b83ffff13820c3c07194f5db48e54d8c8b1d09db632`와 full 25-path digest `ba938518beca8f3718abdf4eb44430e26b3a775aea8f8b33dc5dc01937218f23`가 모두 불변이었다. P4-A source/integration baseline은 baseline `2d5c02dbc612f9b612f27a36263b95e842c24e75`의 direct descendant인 `aeedceb2c3b7008439a9c72e3984be77f6135e51` (`feat: complete P4-A offline synthetic evaluator`)이다. Fresh fetched-`origin/main` integration worktree에서 `git merge --ff-only aeedceb…`와 non-force push가 성공했고, capture time에 integration HEAD/`origin/main`/remote main은 모두 `aeedceb…`였으며 ancestry와 양쪽 worktree clean state가 검증됐다. 이 nine-file documentation closure를 포함하는 commit은 `aeedceb…`의 newer descendant이고, exact SHA와 remote-tip 상태는 Git history가 정본이다. `aeedceb…`는 continuing tip이 아니라 P4-A source/integration baseline으로 남는다. Synthetic 성공은 실제 OCR/Provider 품질, 대표 코퍼스 승인, 전체 P4/P5, 제품·운영 승인 또는 production readiness를 뜻하지 않는다. P4-B는 `BLOCKED_QUALITY_CORPUS_APPROVAL`, P4-C는 독립적으로 `BLOCKED_AP02_PROVIDER_OPT_IN`이며 P4-A는 어느 gate도 열지 않는다. 실 PDF/XLS/XLSX, 외부 OCR/AI/Provider·credential·network, 실데이터 apply/import, production/non-disposable migration·DB-role activation, 배포·release·public service는 계속 미승인이다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 production 활성화 전 재검토할 accepted debt다.
