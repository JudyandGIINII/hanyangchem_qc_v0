...
> **구현 증거 상태 (2026-08-02):** P0A/P0B/P1/P2/P3 source increments는 complete·accepted이고 P3 source commit `91465f0413d0c0ca2633577078ec1300a6096442`는 fresh-main fast-forward 통합 후 `origin/main`에 전달됐다. P4-A Offline/Synthetic foundation은 strict/versioned schema, canonical Decimal/SHA-256 binding, deterministic 8-stage runner, independent candidate payload, 20-edge disposition matrix, fail-closed geometry/normalization/warning 처리와 재현 가능한 metrics/report 범위에서 구현됐다. 최신 remediation은 non-success extraction의 observed warning을 fail closed하고 Decimal geometry/ratio에 precision 28과 `ROUND_HALF_EVEN`을 고정했으며 worker verification은 통과했다. 이 source/doc mutation으로 이전 review는 무효이므로 controller QA와 fresh independent read-only review가 다시 통과해야 acceptance/Git delivery로 진행할 수 있다. P4-A는 base HEAD `2d5c02dbc612f9b612f27a36263b95e842c24e75` 위 uncommitted/unintegrated/unpushed candidate이며 Git delivery를 완료했다고 주장하지 않는다. Pre-doc 16-path source freeze SHA-256은 `cf30f5c1cfad535143a0ed7fe8002e44e84ec0a67aaa9dcf15d06e32edfd5541`이고 후속 source/doc 변경 뒤 full-candidate hash가 아니다. Synthetic 성공은 실제 OCR/Provider 품질, 대표 코퍼스 승인, 전체 P4/P5, 제품·운영 승인 또는 production readiness를 뜻하지 않는다. P4-B는 `BLOCKED_QUALITY_CORPUS_APPROVAL`, P4-C는 독립적으로 `BLOCKED_AP02_PROVIDER_OPT_IN`이다. 실 PDF/XLS/XLSX, 외부 OCR/AI/Provider·credential·network, 실데이터 apply/import, production/non-disposable migration·DB-role activation, 배포·release·public service는 계속 미승인이다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 production 활성화 전 재검토할 accepted debt다.
