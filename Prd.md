...
> **Local-only OCR accepted delivery (2026-08-03):** 외부 OCR/AI 없이 명시적 로컬 PaddleOCR 모델만 사용하는 native-text-first 저화질 PDF 파이프라인의 authorized engineering scope는 complete·accepted·committed·fresh-main fast-forward integrated·`origin/main` delivered 상태다. Source/integration baseline은 `91fd4a8229b12d2b229f2ef9abb9dceef93591b5`; 이 closure 직전 main baseline `96413d20230b62033ecb754a12e5a1a621a7b95c`는 그 후손이며 closure commit은 더 최신 후손이 된다. Exact live tip은 Git history가 정본이다. Final independent review는 `ACCEPT_WITH_MINOR` (BLOCKER 0, MAJOR 0, MINOR 4, NOTE 8)였고 B1/M1-M9와 MA-1은 closed다. 네 residual minor—broad/over-inclusive native table signal, fake-backend-only native low-confidence evaluator wiring, review-time post-remediation smoke non-rerun(이후 Hermes final-source smoke는 성공), duplicate native word extraction—는 고치지 않은 bounded limitation으로 유지한다. 모든 결과는 `review_required=true`인 candidate이고 자동 확정·승인·판정 또는 external fallback은 없다. P4-B는 real representative-corpus quality claim 전의 `BLOCKED_QUALITY_CORPUS_APPROVAL` gate이며 unfinished code debt가 아니다. P4-C는 deferred/not selected/not approved이고 local-only architecture에 필요하지 않다. 세부 범위·실행 증거·남은 제한·금지는 [`docs/research/2026-08-03-local-only-low-quality-pdf-ocr.md`](docs/research/2026-08-03-local-only-low-quality-pdf-ocr.md)를 따른다.

> **Public synthetic-demo boundary (2026-08-03):** `96413d2…`의 independent review는 `ACCEPT_WITH_MINOR` (BLOCKER 0, MAJOR 0, MINOR 6)였다. Residual은 vacuous static approval assertion, runtime fetch-spy/effect coverage 부재, formatting-brittle source slicing, server-oriented public status copy 가능성, flag/Root Directory를 pin하는 committed `vercel.json`/`.vercelignore` 부재, root `.env.example`이 Compose web image build에 주입되지 않는 점이며 고치지 않았다. Controller frontend verification은 36 passed plus build였다. Verified production deployment는 Vercel project `hanyangchem_qc`의 `dpl_2AJpKy3L7ZLiBgEx3LRqXnxDBb7Y` (`READY`, `sourceCommit=cf6d6327172fb09da0fe0e3b12159f6596553c41`)이며 URL `https://hanyangchem-739r15g9t-judy-ng-ii-nii-s-projects.vercel.app`와 production alias `https://hanyangchemqc.vercel.app`가 이를 가리킨다. Next.js 15.5.22 build는 compile/lint/type validity, five static routes와 `/api/health` route를 57s 안에 완료했고 root HTTP 200 및 `/api/health` `{"status":"ready"}`를 확인했다. Alias browser QA는 title `HANYANG QUALITY | 입고 검사`, public/no-backend/no-server-persistence boundary, `Failed to fetch` 없음, `localhost`/`127.0.0.1` resource 없음, alias-only resource host, console message/error 0, 그리고 team-lead 화면의 no API approval button/local synthetic approval button을 확인했다. 따라서 이 exact deployment에 한해 `NEXT_PUBLIC_HYC_PUBLIC_DEMO=1`은 synthetic frontend-only이고 internal/backend API call이나 server persistence가 없으며 backend/DB/worker/OCR/model/original documents는 local/intranet-only다. Production/Preview flag는 저장됐다. Missing/incorrect future flag 또는 project configuration은 localhost-fetch mode로 fall back할 수 있으므로 해당 future deployment는 deployment API와 browser network/behavior를 다시 검증해야 한다. 이 post-deployment documentation commit은 `cf6d632…`의 later docs-only descendant이고 `frontend/` tree는 unchanged이므로 verified deployment가 relevant frontend artifact이며 unchanged frontend bytes만으로 rebuild할 필요는 없다; later docs-only tip은 Git history가 정본이다. Reviewer evidence는 Vitest 36, `tsc`, `eslint`이며 build/Playwright는 reviewer가 실행하지 않았다.

> **구현 증거 상태 (2026-08-02):** P0A/P0B/P1/P2/P3 source increments는 complete·accepted이고 P2/P3는 `origin/main`에 전달됐다. P4-A Offline/Synthetic foundation도 strict/versioned schema, canonical Decimal/SHA-256 binding, deterministic 8-stage runner, independent candidate payload, 20-edge disposition matrix, fail-closed geometry/normalization/warning 처리와 재현 가능한 metrics/report 범위에서 complete·independently accepted·committed·fresh-main fast-forward integrated·delivered 상태다. Original source/integration baseline은 `aeedceb2c3b7008439a9c72e3984be77f6135e51`이고 post-integration documentation ancestors는 `1d98e4cf17b37e0ea95eadcbe69418778d1a614f`와 `afe58a0fe556e8ae94b11926dd572ef9b2e60ee5`다. Pre-P4-B maintenance commit `cad1ab48b7ab1923638fe8600f23ef640efdab73` (`fix: stabilize P4-A geometry validation`)은 `afe58a0…`의 direct descendant로 fresh `origin/main` fast-forward와 non-force push로 delivered 됐다. Capture time에 integration HEAD/`origin/main`/remote main은 모두 `cad1ab4…`였고 ancestry와 clean status가 통과했다. Exact four-path maintenance candidate의 review freeze digest `7ed91a678ba1dd72c30f7e9b58d5e5066fdcf41f8cde2b9da8239447345a85ce`는 review 전후 동일했고 final verdict는 `ACCEPT` (blocker 0, major 0, minor 0)였다. 이 maintenance는 schema Decimal-context와 dead internal polygon-counter notes를 닫았다. Polygon validation arithmetic은 precision 28 / `ROUND_HALF_EVEN`으로 고정되고 public `missing_polygon_count`/`invalid_polygon_count`는 Literal-zero compatible하며 strict required geometry와 fail-closed semantics는 유지된다. Controller evidence는 focused 9, P4 golden 192, Ruff, strict mypy 49/0, backend 510 passed/77 deselected, compileall, full `make check` exit 0, frontend Vitest 32/build, migration contract 4, scans/Compose이며 benchmark를 두 번 반복해 output/report/fixture digests `354d7c10…bfd23` / `7b0601d2…8933` / `05f77739…18d0`가 불변임을 확인했다. 이후 documentation-only closure의 exact commit SHA와 remote-tip 상태는 Git history가 정본이다. 이 accepted/delivered hygiene는 실제 OCR/Provider 품질, P4-B/P4-C, 전체 P4/P5, 제품·운영 승인 또는 production readiness를 뜻하지 않는다. P4-B는 `BLOCKED_QUALITY_CORPUS_APPROVAL`, P4-C는 독립적으로 `BLOCKED_AP02_PROVIDER_OPT_IN`이다. 실 corpus/OCR, 외부 Provider·credential·network, DB/API/UI/service change, 실데이터 apply/import, production/non-disposable migration·DB-role activation, 배포·release·public service는 계속 미승인이다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 production 활성화 전 재검토할 accepted debt다.
