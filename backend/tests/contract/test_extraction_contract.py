...
def test_generated_schema_accepts_canonical_decimal_strings() -> None:
    ...
    assert schema["$defs"]["ExtractionValue"]["properties"]["normalized_value"]["anyOf"][0][
        "pattern"
    ] == CANONICAL_DECIMAL_STRING_PATTERN
...
def test_canonical_decimal_strings_validate(value: str) -> None:
    ...
    payload["values"][0]["normalized_value"] = value  # type: ignore[index]
...
@pytest.mark.parametrize("mutator", [
    ...
    lambda payload: payload["values"][0].update(confidence=1.1),  # type: ignore[index]
    lambda payload: payload["values"][0]["provenance"]["bbox"].update(right=0.0),  # type: ignore[index]
    lambda payload: payload["values"][0].update(normalized_value=1.2),  # type: ignore[index]
    lambda payload: payload.update(candidate_id="not-a-uuid"),
    lambda payload: payload.update(created_at="2026-07-31T00:00:00+09:00"),
])
def test_invalid_contract_inputs_fail_closed(mutator: object) -> None:
    payload = candidate_payload()
