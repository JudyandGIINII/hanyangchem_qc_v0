...
- `docs/plans/2026-07-30-integrated-implementation-plan.md` is the approved delivery contract through P3: P0A through P3 are complete and accepted at their source gates, and P3 is committed, fresh-main fast-forward integrated, and delivered to `origin/main`. Git history is authoritative for the live tip.
...
- Current state is `P0A_P0B_P1_P2_P3_SOURCE_COMPLETE_ACCEPTED_P3_DELIVERED_TO_ORIGIN_MAIN`.
...
- Under the user's explicit 2026-08-02 authorization, Hermes/controller may QA and deliver this six-document kickoff handoff with a normal commit and non-force push, and the next session may start only P4-A offline/synthetic foundation. P4-A is limited to generated non-sensitive synthetic fixtures and existing fixture-provider seams; real data, external Providers, credentials, network calls, deployment, migrations, DB/service changes, and production activation remain unauthorized. P4-B remains blocked until QUALITY approves corpus representativeness and de-identification evidence; P4-C remains independently blocked until a provider-specific AP-02 opt-in is approved.
