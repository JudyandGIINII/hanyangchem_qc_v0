...
import { areMatchesConfirmedForFixture, areRequiredTestsCompleteForFixture, canApprove, canReturn, canSubmit, getInternalTestSemantics, isCanonicalDecimalString, isDocumentReviewCompleteForFixture, reduceInspection } from "../../lib/inspection/workflow";
...
export function InspectionWorkspace({ publicDemo = PUBLIC_DEMO_MODE }: { publicDemo?: boolean }) {
  ...
  const returnGuard = canReturn(state, reviewReason);
  ...
  return <main className="workspace-shell">
    ...
    <section className="content-area">
      ...
      {publicDemo ? <section className="form-card" aria-label="공개 합성 데모 경계"><p className="eyebrow">PUBLIC SYNTHETIC FEEDBACK DEMO</p><h2>읽기 전용 서버 경계</h2><Notice>이 공개 화면은 합성 fixture와 브라우저의 local reducer만 시뮬레이션합니다. 실제 문서가 없고 백엔드·DB·worker·OCR·모델에 연결하지 않으며 서버에 저장하지 않습니다.</Notice></section> : <section className="form-card" aria-label="P3 API 실행 제어"><p className="eyebrow">POSTGRESQL-BACKED P3 VERTICAL SLICE</p><div className="button-row">
        ...
        <button data-testid="confirm-review" className="secondary-button" type="button" disabled={busy || !documentRecord || !extractionRun || !intakeRecord || extractionRun.status === "CONFIRMED"} onClick={() => void runApi("review/match", async () => setExtractionRun(await confirmReview(tokenFor("INSPECTOR")!, documentRecord!.document_id, extractionRun!, intakeRecord!.allocation_id)))}>4. 검토·매칭 확정</button>
