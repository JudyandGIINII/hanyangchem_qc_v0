...
**상태:** P0A/P0B/P1/P2/P3 source complete and accepted; P2/P3 are delivered to `origin/main`. P4-A Offline/Synthetic is complete/accepted/delivered at original source/integration baseline `aeedceb2c3b7008439a9c72e3984be77f6135e51`, followed by documentation ancestors `1d98e4cf17b37e0ea95eadcbe69418778d1a614f` and `afe58a0fe556e8ae94b11926dd572ef9b2e60ee5`. Pre-P4-B maintenance `cad1ab48b7ab1923638fe8600f23ef640efdab73` directly descends from `afe58a0…`, was independently accepted `ACCEPT` (blocker 0, major 0, minor 0), and was delivered by fresh `origin/main` fast-forward/non-force push. Its exact four-path digest stayed `7ed91a678ba1dd72c30f7e9b58d5e5066fdcf41f8cde2b9da8239447345a85ce` before/after review; capture-time integration HEAD/`origin/main`/remote main all equaled `cad1ab4…`, ancestry and clean status passed. Git history is authoritative for the exact later documentation-only SHA and remote-tip state. P4-B is independently `BLOCKED_QUALITY_CORPUS_APPROVAL`; P4-C is independently `BLOCKED_AP02_PROVIDER_OPT_IN`. No full-P4/P5 completion, deployment, release, production readiness, real data/OCR, Provider/network/credential, DB/API/UI/service, production migration, or production DB-role claim is authorized.
**정본:** `Prd.md`  
...
|AP-02 external-provider gate|P4-C OPS/QUALITY/HERMES-QA|One complete [`P4-C Provider-specific AP-02 packet`](approvals/P4C_PROVIDER_AP02_DECISION_PACKET.md) per Provider; generic/cross-provider approval forbidden|`BLOCKED_AP02_PROVIDER_OPT_IN`; `PENDING / NOT APPROVED`|
...
현재 매트릭스는 P0A/P0B/P1/P2/P3 accepted source와 delivered P2/P3 Git evidence를 보존한다. P4-A Offline/Synthetic도 original baseline `aeedceb…`와 accepted/delivered maintenance `cad1ab4…`까지 complete다. Maintenance는 former schema Decimal-context/dead-counter notes를 닫은 hygiene이며 P4-B/P4-C/full P4 completion이 아니다. Exact later documentation-only commit SHA와 remote-tip 상태는 Git history가 정본이다. P4-B와 P4-C는 각각 QUALITY corpus approval과 Provider-specific AP-02 opt-in이 없어 독립적으로 blocked다. Linked packet templates are preparation-only and remain `PENDING / NOT APPROVED`; named approval 전 real corpus benchmark, adapter implementation, external Provider invocation은 금지된다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 accepted debt이며 production 활성화 전에 재검토한다. 실데이터 apply·import, 외부 OCR·AI/NAS/Drive/ERP, 비일회성/production DB, production DB-role activation, 배포·release·서비스 공개·production readiness는 계속 blocked/미승인이다.
