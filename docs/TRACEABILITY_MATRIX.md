...
**2026-08-10 P5 갱신:** P5 Core MVP는 21개 `P5` 태그 행 중 12행 구현으로 **미완료**다. 완료: FR-MST-001/002/003/004/005, FR-SPEC-002, FR-NCR-001/002/004, FR-MAP-001, FR-APR-003, FR-INT-006(첨부 링크로 저장 계층 충족). FR-NCR-003은 기존 `inspection_cases` lineage로 충족되어 신규 작업이 없었다. 미완료 5행은 전부 승인 게이트다: FR-SPEC-003(QUALITY review), FR-SPEC-007(항목 정책 QUALITY 승인), FR-MAP-003(QUALITY approval), FR-OCR-001(AP-02 + QUALITY benchmark, P4-B/P4-C 연쇄), FR-INT-003(FR-SPEC-007과 동일 게이트). 판정 임계값과 샘플링 규칙은 실제 합부 판정에 쓰이므로 임의 생성하지 않았다. 인용 경로 정정 2건: FR-INT-002가 인용한 `test_internal_results_api.py`는 존재하지 않으며 실제 커버리지는 `test_vertical_slice.py`와 `test_internal_substitute_hold.py`에 있다. P5 인계 상태는 `HANDOFF.md`의 2026-08-10 섹션이 정본이다.
**2026-08-11 P6 갱신:** 사용자 결정으로 **P6 Operations 전체가 승인**됐다. 범위·설계 정본은 [`plans/2026-08-11-p6-operations-scope-plan.md`](plans/2026-08-11-p6-operations-scope-plan.md)이다. 승인 대상 행: REP-001/002/003/004, UI-005, API-002(report Idempotency), API-003(Job ID), FR-DOC-001/002/005, ARCH-002/003, BACKUP-001, NFR-AVAIL(status/수동 재시도 한정), 그리고 PRD Phase 2의 부적합 후속조치·마스터 Import. **제외:** RET-001 보존 정책(AP-08 계속 미승인, 사용자 결정으로 범위 밖). 승인의 성격은 코드 작성이며 **실물 접근이 아니다** — 사용자가 COA 샘플·NAS/Drive 접속·Import용 실 엑셀의 부재를 명시하고 "준비만 해"를 지시했으므로 FR-DOC-001/002/005, 마스터 Import, OCR 운영 모니터링은 포트/계약 확정 + 합성 구현체 CI 통과 + 어댑터 국소화 + 플래그 off 기본의 4조건을 만족하는 seam까지다. 실제 NAS/Drive/ERP 호출, 자격증명, 실 PDF/XLSX 커밋(AP-05), 외부 Provider 호출(AP-02), production migration/DB-role은 계속 미승인이다. **승인으로 풀리지 않는 사실상의 제약:** OCR 품질 KPI 임계값은 PRD §3.3이 "초기 운영 데이터 확보 후 기준선 측정"을 요구하므로 COA 샘플 없이 생성하지 않으며, 이 행들의 Gate 열에 적힌 QUALITY 판단(REP-001 sample approval 포함)은 열 구조 호환성 확보와 별개로 유지된다. P5 잔여 5행(FR-SPEC-003/007, FR-MAP-003, FR-OCR-001, FR-INT-003)의 QUALITY/AP-02 게이트도 P6가 해제하지 않는다. 설계상 확정된 중요 사항: `decision_snapshots`에 품목·공급사·모델 명칭, COA 메타, 부적합, 첨부, 검사자가 **없으므로** REP-002는 판정값만 스냅샷에서 읽고 참조 정보는 조회 시점 값을 쓰되 보고서에 출처를 명시 렌더한다. 스냅샷 스키마는 확장하지 않는다 — `APPROVAL_SNAPSHOT_REQUIRED_KEYS` 변경은 기존 `content_hash` 계약을 깨고, 부적합은 승인 이후 발생하는 사후 사건이라 승인 스냅샷에 얼리는 것이 모델링상 틀리기 때문이다.

**2026-08-11 P6-1 구현:** REP-002 통합 검사보고서와 그 실행 틀이 구현됐다(브랜치 `JudyandGIINII/p6-1-report-foundation-20260811`, main 미통합). 갱신되는 행: **REP-002**는 `backend/src/hyc_api/reports/integrated.py`와 `backend/tests/unit/reports/test_integrated_report.py`가 근거이며 승인 스냅샷 기준 생성과 정정 시 새 버전 생성을 만족한다. **API-002**의 report Idempotency 잔여 범위는 `backend/tests/integration/api/test_report_job_lifecycle.py`가 닫는다(동일 키·동일 페이로드 byte-identical 재생, 동일 키·다른 페이로드 409, 다른 키 재생성은 새 job). **API-003**의 Job ID 요구는 `POST /api/v1/reports` 202 + `GET /api/v1/reports/{job_id}` 폴링으로 충족된다. **SEC-001**의 download audit은 `routes/reports.py`가 다운로드마다 `REPORT_DOWNLOADED` AuditLog 행을 남겨 부분 충족한다. **ARCH-003**의 원본/파생 구분은 `report_artifacts`를 `documents`와 분리하고 별도 저장소 루트를 쓰는 것으로 충족한다. 설계 D4에 따라 판정값만 `decision_snapshots`에서 읽고 명칭·COA 메타·부적합·첨부는 조회 시점 값을 쓰되 시트에 출처를 렌더한다. 미충족으로 남는 것: **REP-001**(Raw Data 호환 시트), **REP-003**, **REP-004**, **UI-005**는 P6-2 범위이며 이번 증분에 없다. **NFR-PERF**의 report async는 계약상 충족이나 실행기가 in-process이므로 부하 프로파일 측정은 하지 않았다. 검증: `make check` exit 0(mypy 86/0, backend 720/178, frontend Vitest 65/9, migration 4, scans, compose), `p6-report-check` 15, P2 PostgreSQL 21, P3 PostgreSQL 140(이전 125), P4 golden 199·preflight 97 불변. 주의로 기록: 신규 통합 테스트는 `pytestmark = pytest.mark.postgres`가 없으면 일반 실행에서 skip되고 postgres 게이트에서 deselect되어 어느 게이트에서도 실행되지 않는다. 이번 증분에서 실제로 4건이 그 상태로 작성됐다가 교정됐다.

**2026-08-11 P6-2 구현:** REP-001, REP-004, UI-005가 구현되고 REP-003은 차단됐다(브랜치 `JudyandGIINII/p6-1-report-foundation-20260811`, main 미통합). **REP-001**은 `backend/src/hyc_api/reports/raw_data.py`와 `backend/tests/unit/reports/test_raw_data_report.py`가 근거이며, 열 구조 확보 절차와 그 한계는 [`evidence/2026-08-11-rep001-raw-data-column-structure.md`](evidence/2026-08-11-rep001-raw-data-column-structure.md)에 있다. 원본 템플릿이 56열에서 잘려 검사항목 9개까지만 담는 결함을 확인했고 구현은 이를 넘겨 확장한다(20개 → 113열, 부분 그룹 0). **Gate는 여전히 `QUALITY sample approval`이다** — 열 구조 호환성만 확보했을 뿐 출력물이 품질팀이 원하는 모양인지는 실 산출물 검토 후에만 확인된다. **REP-004**와 **UI-005**는 `reports/statistics.py`, `routes/statistics.py`, `frontend/src/components/statistics/StatisticsPanel.tsx`가 근거다. 모집단은 `decision_snapshots` 행이 있는 승인 완료 건이며 `CANCELLED`은 제외한다. 화면과 엑셀은 단일 집계 엔진을 공유한다. **PRD §12.5의 "테스트 데이터 제외"는 미충족으로 남는다** — 스키마에 테스트 레코드를 식별할 표시가 없어 구현 불가이며 플래그를 임의로 만들지 않았다. **PRD §16.4의 OCR 검토 필요율도 미충족이다** — `review_required`가 케이스별로 영속되지 않아 유도할 수 없고, 조작된 0%가 human-review 불변식과 정반대를 주장하므로 `미측정` 센티널로 표시한다. 실제 측정하려면 케이스별 검토 필요 여부를 저장하는 스키마 변경이 필요하다. **REP-003은 `BLOCKED_INCOMPLETE_GENERATOR`다** — 생성기가 승인 스냅샷에 없는 `allocations` 키를 읽어 분할 입고 시트가 실데이터로 항상 비므로 `REPORT_KIND_NOT_IMPLEMENTED`로 fail closed 시켰다. **NFR-PERF 관련 기록:** 통계 조회는 기간을 SQL로 밀어 넣어야 한다. 기간 없이 전체 승인 이력을 적재하던 초기 배선에서 P3 게이트가 32초 → 600초 초과로 악화됐고, KST 창을 UTC 경계로 변환해 필터링하도록 고쳐 32초로 복구했다. 검증: `make check` exit 0(mypy 90/0, backend 734/185, frontend 69/10, migration 4, scans, compose), `p6-report-check` 33, P2 PostgreSQL 21, P3 PostgreSQL 143, P4 golden 199·preflight 97 불변.

**2026-08-12 P6-2 종료:** 이전 항목의 두 미충족이 해소됐다. **REP-003은 더 이상 차단이 아니다** — `load_lot_trace_sources`가 LOT의 전체 allocation을 DB에서 읽어 분할 입고를 렌더하며, 입고 2건이 정확히 2개 행으로 나오는 회귀가 이를 고정한다. **PRD §16.4 OCR 검토 필요율도 충족됐다** — 직전 기록의 "스키마에 영속되지 않아 유도 불가"는 오판이었고, `extraction_runs.status`가 검토 상태를 이미 영속하므로 마이그레이션 없이 측정된다. 미충족으로 남는 것은 **PRD §12.5의 테스트 데이터 제외** 하나뿐이며, 스키마에 테스트 레코드 식별 표시가 없어 계속 구현 불가다(플래그를 임의로 만들지 않았다). REP-001의 `QUALITY sample approval` 게이트도 그대로다 — 열 구조 호환성만 확보했다. UI-005는 `InspectionWorkspace`의 `보고서·통계` 단계로 마운트됐고, 공개 데모 경계는 해당 탭 이동 후에도 fetch 0회임을 회귀로 고정한다. **환경 기록 갱신:** Playwright는 `COMPOSE_BAKE=false`로 더 이상 통과하지 않으며 `COMPOSE_BAKE=0 DOCKER_BUILDKIT=0`가 필요하다. 검증: `make check` exit 0(mypy 90/0, backend 740/181, frontend 70/10, migration 4, scans, compose), `p6-report-check` 35, P2 PostgreSQL 21, P3 PostgreSQL 143, Playwright 3/3, P4 golden 199·preflight 97 불변.
...
**2026-08-13 1차 MVP 종료:** 엔지니어링으로 진행 가능한 행이 남지 않았다. 미충족으로 남는 행은 전부 승인 또는 실물 부재가 원인이며 코드 부채가 아니다 — **PRD §12.5 테스트 데이터 제외**(스키마에 식별 표시 없음), **REP-001 QUALITY sample approval**(열 구조만 확보), **FR-OCR-001·P4-B·P4-C**(시험 샘플·AP-02), **FR-SPEC-003/007·FR-MAP-003·FR-INT-003**(QUALITY 정책), **RET-001·FR-DOC-005 mirror·ERP 실연계**(AP-06/AP-08). **추가 개발은 시험 샘플 확보와 NAS/Google Drive 접속 확인 후 재개한다.** 인계 정본은 [`HANDOFF.md`](HANDOFF.md)의 2026-08-13 섹션이다. **미검증 경고:** 이번 작업에서 매트릭스가 존재하지 않는 테스트를 인용한 사례가 3건 확인됐다(P5 `test_internal_results_api.py`, FR-MST-006 `test_erp_bom_seam.py`, REP-003 `test_production_lot_link_seam.py`). 셋 다 '구현 완료'로 기록돼 있었다. **남은 행의 인용 경로는 전수 검증되지 않았으므로 실재를 확인하기 전까지 근거로 신뢰하지 말 것.**
