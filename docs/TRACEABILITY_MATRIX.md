...
|Public synthetic demo boundary|WEB/OPS|`96413d2…` independent review `ACCEPT_WITH_MINOR` 0/0/6: vacuous static approval assertion; no runtime fetch-spy/effect coverage; formatting-brittle source slicing; possibly server-oriented public status; no committed `vercel.json`/`.vercelignore` pinning flag/Root Directory; root `.env.example` does not feed Compose web build. None fixed. Controller frontend 36 passed/build; reviewer Vitest 36, `tsc`, `eslint` only. Missing/incorrect flag/configuration can fall back to localhost fetch; saved Production/Preview flag still requires rebuild/redeploy and browser-network proof. Deployment API plus browser observation—not this matrix—are authoritative for live identity/behavior.|Source boundary delivered; no backend/DB/worker/OCR/model/document exposure; live state unverified|
