...
**상태:** P0A/P0B/P1/P2/P3 source complete and accepted; P2/P3 are delivered to `origin/main`. P4-A Offline/Synthetic is also complete, independently accepted, committed, fresh-main fast-forward integrated, and delivered at source/integration baseline `aeedceb2c3b7008439a9c72e3984be77f6135e51`, a direct descendant of `2d5c02dbc612f9b612f27a36263b95e842c24e75`. Final review was `ACCEPT_WITH_MINOR` (BLOCKER 0, MAJOR 0, MINOR 3) with unchanged 16-path source-only digest `0d5c259f59293f35e4cf6b83ffff13820c3c07194f5db48e54d8c8b1d09db632` and 25-path full-candidate digest `ba938518beca8f3718abdf4eb44430e26b3a775aea8f8b33dc5dc01937218f23` before/after review. `aeedceb…` and the full-candidate digest are capture-time delivery/acceptance evidence. The commit containing this nine-file documentation closure is a newer descendant; Git history is authoritative for its exact SHA and remote-tip status, while `aeedceb…` remains the P4-A source/integration baseline rather than the continuing tip. P4-B is independently `BLOCKED_QUALITY_CORPUS_APPROVAL`; P4-C is independently `BLOCKED_AP02_PROVIDER_OPT_IN`. No full-P4/P5 completion, deployment, release, production readiness, real data, external service, production migration, or production DB-role claim is authorized.
**정본:** `Prd.md`  
...
|Metrics and geometry|P4-A WORKER/QUALITY|Exact/normalized, row precision/recall, missing confusion, by-kind, page and polygon Decimal IoU metrics under explicit scorer precision 28 / `ROUND_HALF_EVEN`; ambient rounding/precision cannot change canonical output; `PAGE_MISMATCH` has no IoU, no invented threshold; concave, self-intersecting, degenerate, malformed geometry fail closed. Accepted minor: schema-level polygon validation arithmetic is not within that scorer context and must close before P4-B real-corpus geometry expansion.|P4-A accepted/delivered synthetic scope; acceptance threshold remains QUALITY decision|
|Commands and reproducibility|P4-A WORKER/HERMES-QA|Final controller focused selector 15/47; golden 183; Ruff; backend 501/77, strict mypy 49 files/0 errors, compileall; full check exit 0 with frontend lint/typegen/typecheck, Vitest 32/build, migration 4, scans/Compose; P2/P3 PostgreSQL 10/67; cleanup 0/0/0. Benchmark output/report/fixture `354d7c10…fd23` / `7b0601d2…8933` / `05f77739…18d0`. P3 browser E2E was not run because no runtime/API/UI/workflow path changed.|Final controller QA passed; final review `ACCEPT_WITH_MINOR` 0/0/3|
|Candidate/Git boundary|P4-A HERMES-QA|Final review preserved source-only 16-path digest `0d5c259f…db632` and full 25-path digest `ba938518…18f23`. Source commit `aeedceb…` directly descends from `2d5c02d…`; fresh fetched-`origin/main` worktree fast-forward integration and non-force push succeeded without merge commit/rebase. Capture-time integration HEAD/`origin/main`/remote equality, ancestry, and clean implementation/integration worktrees passed. The commit containing this nine-file closure is a newer descendant, whose exact SHA/remote-tip status comes from Git history.|Complete/accepted/delivered; `aeedceb…` is source/integration baseline, not continuing tip|
...
|P4-A accepted minor notes|P4-A WORKER/QUALITY|Strict required geometry makes `missing_polygons`/`invalid_polygons` counters currently unrepresentable; clean these dead counters before optional-geometry semantics change. Schema polygon arithmetic context gap must close before P4-B geometry expansion. DEVLOG cross-reference wording is corrected in this docs closure.|Accepted MINOR notes; not blocker/major and not claimed source-fixed|
|P4-A commands|P4-A WORKER/HERMES-QA|Final focused selector 15/47; `make p4-golden-check` 183; byte-identical benchmark digests; full backend/check and P2/P3 PostgreSQL gates passed; cleanup 0/0/0. P3 browser E2E explicitly not run because no runtime/API/UI/workflow path changed.|Final controller QA and independent acceptance passed; delivered|
...
현재 매트릭스는 P0A/P0B/P1/P2/P3 accepted source와 delivered P2/P3 Git evidence를 보존한다. P4-A Offline/Synthetic도 위에 한정된 schema/runner/scorer/metric/edge 범위에서 complete/independently accepted/committed/fresh-main fast-forward integrated/delivered 상태다. Source/integration baseline `aeedceb…`는 base `2d5c02d…`의 direct descendant다. 이 nine-file closure를 포함하는 commit은 `aeedceb…`의 newer descendant이고 exact SHA/remote-tip 상태는 Git history가 정본이며, `aeedceb…`는 continuing tip이 아니다. P4-B와 P4-C는 각각 QUALITY corpus approval과 Provider-specific AP-02 opt-in이 없어 독립적으로 blocked이며, P4-A verification은 어느 gate나 real OCR/Provider benchmark를 충족하지 않는다. 다음 gate는 두 decision/evidence packet 준비이며 named approval 전 real corpus benchmark나 external Provider invocation은 금지된다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 accepted debt이며 production 활성화 전에 재검토한다. 실데이터 apply·import, 외부 OCR·AI/NAS/Drive/ERP, 비일회성/production DB, production DB-role activation, 배포·release·서비스 공개·production readiness는 계속 blocked/미승인이다.
