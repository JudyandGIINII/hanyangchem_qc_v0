...
**상태:** P0A/P0B/P1/P2/P3 source complete and accepted. P2 and P3 are committed, fresh-main integrated, and delivered to `origin/main`; Git history is authoritative for that delivered live tip. P4-A Offline/Synthetic remediation is source complete and worker-verified, but the latest source/doc mutations require controller QA and a fresh independent read-only review before acceptance and Git delivery; it is still uncommitted, unintegrated, and unpushed. Its base HEAD is `2d5c02dbc612f9b612f27a36263b95e842c24e75`; the exact 16-path pre-doc source freeze is `cf30f5c1cfad535143a0ed7fe8002e44e84ec0a67aaa9dcf15d06e32edfd5541`, not a post-remediation full-candidate hash. P4-B is independently `BLOCKED_QUALITY_CORPUS_APPROVAL`; P4-C is independently `BLOCKED_AP02_PROVIDER_OPT_IN`. No full-P4/P5 completion, deployment, release, production readiness, real data, external service, production migration, or production DB-role claim is authorized.
**정본:** `Prd.md`  
...
|Metrics and geometry|P4-A WORKER/QUALITY|Exact/normalized, row precision/recall, missing confusion, by-kind, page and polygon Decimal IoU metrics under explicit precision 28 / `ROUND_HALF_EVEN`; ambient rounding/precision cannot change canonical output; `PAGE_MISMATCH` has no IoU, no invented threshold; concave, self-intersecting, degenerate, malformed geometry fail closed.|P4-A remediation worker-verified; acceptance threshold remains QUALITY decision|
|Commands and reproducibility|P4-A WORKER|Exact focused selector 20/42; golden 183; backend 501/77, mypy 49; frontend 32/build; migration 4; scans/Compose. Benchmark output/report/fixture `354d7c10…fd23` / `7b0601d2…8933` / `05f77739…18d0`; precision 12/28/50 and five-mode rounding sweep passed. `make p3-e2e` was not applicable because no runtime/API/UI/workflow path changed.|Worker verification passed; controller QA and fresh independent read-only review remain|
|Candidate/Git boundary|P4-A HERMES-QA|Base `2d5c02d…`; exact pre-doc 16-path source freeze `cf30f5c…5541`. Later source/documentation changes invalidate that value as a full-candidate hash. No P4-A commit/integration/push is claimed.|Awaiting controller QA and fresh independent review before authorized Git delivery|
...
|P4-A commands|P4-A WORKER/HERMES-QA|Exact focused selector 20/42; `make p4-golden-check` 183; byte-identical `make p4-benchmark-fixture` twice; full `make check` exit 0; scans/network/credential boundary. P3 browser E2E explicitly not applicable because no runtime/API/UI/workflow path changed.|Worker verification passed; controller QA/fresh review and Git delivery pending|
...
현재 매트릭스는 P0A/P0B/P1/P2/P3 accepted source와 delivered P2/P3 Git evidence를 보존한다. P4-A Offline/Synthetic는 위에 한정된 schema/runner/scorer/metric/edge 범위에서 remediation source complete/worker-verified지만, base `2d5c02d…` 위 uncommitted/unintegrated/unpushed candidate이며 controller QA와 fresh independent read-only review를 기다린다. Exact pre-doc 16-path source hash `cf30f5c…5541`은 후속 source/doc 변경 뒤 full candidate hash가 아니다. Acceptance가 다시 확인된 뒤에만 controller Git delivery로 진행한다. P4-B와 P4-C는 각각 QUALITY corpus approval과 Provider-specific AP-02 opt-in이 없어 독립적으로 blocked이며, P4-A verification은 어느 gate나 real OCR/Provider benchmark를 충족하지 않는다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 accepted debt이며 production 활성화 전에 재검토한다. 실데이터 apply·import, 외부 OCR·AI/NAS/Drive/ERP, 비일회성/production DB, production DB-role activation, 배포·release·서비스 공개·production readiness는 계속 blocked/미승인이다.
