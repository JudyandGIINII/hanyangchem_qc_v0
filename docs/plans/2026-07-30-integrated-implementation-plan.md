...
**문서 상태:** `P0A_P0B_P1_P2_P3_SOURCE_COMPLETE_ACCEPTED_P3_DELIVERED_TO_ORIGIN_MAIN_P4A_OFFLINE_SYNTHETIC_COMPLETE_ACCEPTED_DELIVERED_TO_ORIGIN_MAIN`
**정본 요구사항:** `Prd.md`  
...
**권한 경계:** AP-01~05는 승인됐다. P0A/P0B/P1/P2/P3 source increments는 complete·accepted이고 P2/P3는 `origin/main`에 delivered 됐다. P4-A Offline/Synthetic도 final controller QA와 fresh independent read-only review `ACCEPT_WITH_MINOR` (BLOCKER 0, MAJOR 0, MINOR 3)를 통과해 complete·accepted·committed·fresh-main fast-forward integrated·delivered 상태다. Source/integration baseline `aeedceb2c3b7008439a9c72e3984be77f6135e51`은 `2d5c02dbc612f9b612f27a36263b95e842c24e75`의 direct descendant다. Fresh fetched-`origin/main` integration, non-force push, capture-time HEAD/remote equality, ancestry, clean worktrees가 검증됐다. 이 nine-file documentation closure를 포함하는 commit은 `aeedceb…`의 newer descendant이고, exact SHA와 remote-tip 상태는 Git history가 정본이다. `aeedceb…`는 continuing tip이 아니라 P4-A source/integration baseline으로 남는다. P4-B는 `BLOCKED_QUALITY_CORPUS_APPROVAL`, P4-C는 독립적으로 `BLOCKED_AP02_PROVIDER_OPT_IN`, P5는 unstarted다. 실데이터 apply/import, 외부 OCR/AI/Provider, 비일회성/production migration, production DB-role activation, public service exposure, deployment, release, production readiness는 미승인이다. Fixture-only N-1/N-2/N-5와 P2 N-M3는 accepted debt다.
...
AP-01~05는 2026-07-30 권장 기본값대로 승인됐다. P0A/P0B/P1/P2/P3 source increments는 complete·accepted이고 각 source gate가 통과했다. P3는 source commit `91465f0413d0c0ca2633577078ec1300a6096442`로 committed, fresh-main fast-forward integrated, `origin/main`에 delivered 됐다. 2026-08-02 승인 범위의 P4-A Offline/Synthetic도 final QA/review 뒤 source/integration baseline `aeedceb2c3b7008439a9c72e3984be77f6135e51`로 같은 방식으로 delivered 됐다. P4-B/P4-C, AP-06~08의 파일럿/생산 활성화, P5, 실데이터 apply/import, 외부 OCR/AI, 비일회성/production migration, production DB-role activation, 배포와 서비스 공개는 별도 승인 대상이며 현재 blocked/미승인이다.
...
**선행:** accepted P2 (충족). **현재 gate:** P3 구현은 별도 명시적으로 승인됐고 final independent backend/UI/API review와 Hermes controller QA를 통과해 source complete·accepted다. Source commit `91465f0413d0c0ca2633577078ec1300a6096442`는 fresh baseline `b7bc4a8ca258d1d44d240f8884a4b4ec8cbb6abf`에서 fast-forward 통합되어 fresh integrated gate를 통과한 뒤 `origin/main`에 delivered 됐다.
...
**Exact-candidate freeze/Git boundary (historical pre-integration checkpoint):** base HEAD는 `b7bc4a8ca258d1d44d240f8884a4b4ec8cbb6abf`다. Pre-doc-final freeze는 changed/untracked 50 files, source hash `51f3bbb1d23970484813e893e51fd781f89fb781d02fac2db5cb475b00cac7f2`였고, 이 문서 변경 이후 hash라고 주장하지 않는다. 당시 P3는 source accepted이며 exact-candidate commit과 fresh-main fast-forward integration 준비가 됐지만 uncommitted/unintegrated/unpushed였다.

**2026-08-01 post-integration closure (historical P3 state):** P3 source commit `91465f0413d0c0ca2633577078ec1300a6096442` (`feat: complete P3 vertical slice`; exactly 52 files, 8911 insertions, 119 deletions)는 clean local main/`origin/main` baseline `b7bc4a8ca258d1d44d240f8884a4b4ec8cbb6abf`에서 `git merge --ff-only` 방식으로 merge commit·rebase 없이 통합됐다. Fresh integrated bootstrap/check, Ruff, mypy 39, backend 346/77, frontend 32/build, migration 4, scans/Compose, P2/P3 PostgreSQL 10/67, real Playwright 3/3이 모두 통과했고 HYC cleanup은 containers/networks/volumes 0/0/0이며 n8n은 untouched였다. Push `b7bc4a8..91465f0 main -> main` 후 local main, `origin/main`, remote main이 모두 source commit과 같음, base/source ancestry, 당시 main/candidate clean 상태를 검증했다. 당시 P4/P5 unstarted였다는 기록은 이 P3 closure 시점의 역사적 사실이며, 현재 P4-A 상태는 아래 P4 section이 supersede한다. N-1/N-2/N-5와 P2 N-M3는 accepted non-production debt다.

### P4 — OCR Golden/Provider benchmark

**현재 상태 (2026-08-02):** P4-A Offline/Synthetic is complete/accepted/delivered at original source baseline `aeedceb…`; pre-P4-B maintenance `cad1ab4…` is also independently accepted/delivered. P4-B `BLOCKED_QUALITY_CORPUS_APPROVAL`; P4-C `BLOCKED_AP02_PROVIDER_OPT_IN`. Both packet templates are `PENDING / NOT APPROVED`. P4-B/P4-C는 독립 gate이며 P4-A maintenance가 어느 쪽도 충족하지 않는다.

**선행:** P4-A는 P3 이후 generated non-sensitive synthetic 범위에서 완료했다. P4-B만 품질팀의 대표 코퍼스/비식별 승인이 선행하며 P4-C는 별도 Provider-specific AP-02 opt-in이 선행한다.
...
**P4-A final evidence:** strict/versioned golden/fixture/stage/candidate/report/output schema, canonical JSON and Decimal/SHA-256 exact binding, deterministic eight-stage fail-closed runner, independent candidate payloads, executable disjoint 20-edge disposition matrix, canonical warnings/errors, exact/normalized/row/missing/by-kind/page/polygon metrics, and fail-closed invalid geometry. Original final review accepted 25 paths with three disclosed minors and unchanged source-only/full digests `0d5c259f…db632`/`ba938518…18f23`. The later exact four-path maintenance candidate closed the schema Decimal-context and dead internal polygon-counter notes while preserving strict required geometry/fail-closed semantics; public missing/invalid polygon counts remain Literal-zero compatible and polygon arithmetic is precision 28 / `ROUND_HALF_EVEN`. Maintenance review freeze digest `7ed91a67…a85ce` stayed unchanged and final verdict was `ACCEPT` 0/0/0. Controller focused 9, golden 192, Ruff, backend 510/77, strict mypy 49/0, compileall, full check exit 0, frontend Vitest 32/build, migration 4, scans/Compose passed. Two benchmark repetitions retained `354d7c10…bfd23`, `7b0601d2…8933`, `05f77739…18d0`.

**P4-B/P4-C decision artifacts:** [`P4-B QUALITY corpus packet`](../approvals/P4B_QUALITY_CORPUS_DECISION_PACKET.md) and [`P4-C Provider-specific AP-02 packet`](../approvals/P4C_PROVIDER_AP02_DECISION_PACKET.md) are preparation-only templates. Any `PENDING`/missing field keeps the relevant lane blocked; P4-C requires one complete packet per Provider, and an external benchmark requires the intersection of approved P4-B and P4-C scope.
...
|---|---:|---|---|
|AT-001 COA 파싱|P4/P5|`backend/tests/golden/{test_schema.py,test_artifacts_and_scoring.py,test_runner_stages_metrics_edges.py}`|P4-A synthetic 행/LOT/spec/result/page/polygon·저신뢰·reference-only 검증 완료; 대표 실문서는 P4-B/P5|
...
|AT-004 가변 샘플|P4/P5|`backend/tests/golden/{test_schema.py,test_runner_stages_metrics_edges.py}`|P4-A synthetic identity/value/raw/order/cardinality 검증 완료; 대표 실문서는 P4-B/P5|
...
- [x] P3 source commit `91465f0413d0c0ca2633577078ec1300a6096442` fresh-main fast-forward integration, fresh gates, push, and post-fetch equality/ancestry gate
- [x] P4-A Offline/Synthetic implementation/remediation worker gate: strict schemas/bindings, eight-stage runner, independent candidates, 20-edge dispositions, fail-closed warnings, deterministic Decimal metrics/geometry, golden 183, backend 501/77, mypy 49, frontend 32/build, reproducible benchmark and adversarial probes
- [x] P4-A final 25-path freeze/review (`ACCEPT_WITH_MINOR` 0/0/3), source commit `aeedceb…`, fresh-main fast-forward integration, non-force push, and capture-time post-fetch equality/ancestry/clean-state verification
- [x] P4-A four-path pre-P4-B maintenance freeze/review (`ACCEPT` 0/0/0), commit `cad1ab4…`, fresh-main fast-forward/non-force delivery, capture-time equality/ancestry/clean status, golden 192/backend 510/77/full check, and two unchanged benchmark repetitions
- [ ] P4-B QUALITY corpus approval (`BLOCKED_QUALITY_CORPUS_APPROVAL`; packet `PENDING / NOT APPROVED`)
- [ ] P4-C Provider-specific AP-02 opt-in (`BLOCKED_AP02_PROVIDER_OPT_IN`; per-Provider packet `PENDING / NOT APPROVED`)

**현재 상태 (2026-08-02 P4-A maintenance closure):** `P0A_P0B_P1_P2_P3_SOURCE_COMPLETE_ACCEPTED_P3_DELIVERED_TO_ORIGIN_MAIN_P4A_OFFLINE_SYNTHETIC_COMPLETE_ACCEPTED_DELIVERED_TO_ORIGIN_MAIN`. P2/P3와 P4-A는 complete/accepted/committed/fresh-main integrated/원격 delivered 상태다. P4-A original source/integration baseline은 `aeedceb…`; post-integration docs ancestors는 `1d98e4c…`/`afe58a0…`; maintenance source/delivery는 `cad1ab4…`이다. Fresh delivery capture에서 local integration HEAD/`origin/main`/remote main equality, ancestry, clean status가 통과했다. Exact later documentation-only SHA/remote-tip은 Git history가 정본이다. P4-B/P4-C는 independently blocked이고 linked templates는 `PENDING / NOT APPROVED`; P5는 unstarted다. Named approval 전 real corpus benchmark, adapter implementation, external Provider invocation은 금지된다. No P4-B/P4-C code, real corpus/OCR, provider/network/credential, DB/API/UI/service, deployment, migration, or production activation occurred. N-1/N-2/N-5와 N-M3는 accepted non-production debt다.
